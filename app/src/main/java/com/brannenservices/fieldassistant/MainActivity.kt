...
class MainActivity : AppCompatActivity() {
 ...
 private fun buildUi(){
  ...
  body.addView(button("SAFE USB CLAIM TEST — NO FORCE",{safeClaimTest()}))
  deepButton=button("START CONTROLLED RECOVERY SESSION",{startScan()});body.addView(deepButton)
  cancelButton=button("CANCEL RECOVERY SESSION",{scanCancelled=true}).apply{isEnabled=false};body.addView(cancelButton)
  ...
  resultText=TextView(this).apply{text="V0.9 controlled raw session. One USB claim per recovery run. Source SD card remains read-only.";textSize=14f;setTextColor(0xFF9CA3AF.toInt());setPadding(0,14,0,0)};body.addView(resultText)
