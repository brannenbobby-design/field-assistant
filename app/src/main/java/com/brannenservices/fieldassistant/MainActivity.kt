...
class MainActivity : AppCompatActivity() {
    ...
    private fun rawProbe(d:UsbDevice):String{
        ...
        try{if(!c.claimInterface(intf,true))error("Could not claim interface");val cap=readCapacity(c,ep.first,ep.second,1);val s=readBlocks(c,ep.first,ep.second,0,1,cap.blockSize,2);val sig=if(s.size>=512)String.format("%02X%02X",s[510].toInt()and255,s[511].toInt()and255)else"n/a";return "RAW READ SUCCESS\nBlocks: ${cap.blocks}\nBlock size: ${cap.blockSize}\nSignature: $sig\nNo write command sent."}finally{try{c.releaseInterface(intf)}catch(_:Exception){};c.close()}
    }
    ...
    private fun readBlocks(c:UsbDeviceConnection,i:UsbEndpoint,o:UsbEndpoint,lba:Long,count:Int,blockSize:Int,tag:Int):ByteArray{
        val cmd=ByteArray(10);cmd[0]=0x28;cmd[2]=((lba ushr 24)and255).toByte();cmd[3]=((lba ushr 16)and255).toByte();cmd[4]=((lba ushr 8)and255).toByte();cmd[5]=(lba and255).toByte();cmd[7]=((count ushr 8)and255).toByte();cmd[8]=(count and255).toByte();return scsi(c,i,o,cmd,count*blockSize,tag)
    }
    ...
    private fun scsi(c:UsbDeviceConnection,i:UsbEndpoint,o:UsbEndpoint,cdb:ByteArray,dataLen:Int,tag:Int):ByteArray{
        ...
        val b=ByteBuffer.wrap(csw).order(ByteOrder.LITTLE_ENDIAN);if(b.int!=0x53425355)error("Invalid CSW signature");val returnedTag=b.int;if(returnedTag!=tag)error("CSW tag mismatch");val residue=b.int;val st=b.get().toInt()and255;if(st!=0)error("SCSI command failed status=$st residue=$residue");return data
    }
