...
class MainActivity : AppCompatActivity() {
    companion object {
        const val ACTION_USB_PERMISSION = "com.brannenservices.photorescue.USB_PERMISSION"
        const val MAX_JPEG_BYTES = 40 * 1024 * 1024
        const val SECTORS_PER_READ = 128
    }
    ...
    private val receiver = object : BroadcastReceiver() {
        override fun onReceive(c: Context?, x: Intent?) {
            if (x?.action != ACTION_USB_PERMISSION) return
            val d = if (Build.VERSION.SDK_INT >= 33) x.getParcelableExtra(UsbManager.EXTRA_DEVICE, UsbDevice::class.java) else @Suppress("DEPRECATION") x.getParcelableExtra(UsbManager.EXTRA_DEVICE)
            if (d != null && x.getBooleanExtra(UsbManager.EXTRA_PERMISSION_GRANTED, false)) showUsb(d) else status.text = "USB access was not granted"
        }
    }
    ...
    override fun onDestroy() { scanCancelled = true; unregisterReceiver(receiver); super.onDestroy() }
    ...
    private fun buildUi() {
        ...
        r.addView(button("TEST RAW READ — READ ONLY") { rawTest() })
        ...
        resultText = TextView(this).apply { text = "V0.6 BOT recovery + faster scan. Source SD card remains read-only."; textSize = 14f; setTextColor(0xFF9CA3AF.toInt()); setPadding(0,14,0,0) }; r.addView(resultText)
        setContentView(r)
    }
    ...
    private fun capacity(c:UsbDeviceConnection,intf:UsbInterface,i:UsbEndpoint,o:UsbEndpoint,tag:Int):Cap { val d=scsi(c,intf,i,o,byteArrayOf(0x25,0,0,0,0,0,0,0,0,0),8,tag); val b=ByteBuffer.wrap(d).order(ByteOrder.BIG_ENDIAN); return Cap((b.int.toLong() and 0xffffffffL)+1,(b.int.toLong() and 0xffffffffL).toInt()) }

    private fun rawTest() { val d=target() ?: return; if(!usbManager.hasPermission(d)){request(d);return}; progress.visibility=ProgressBar.VISIBLE; progress.isIndeterminate=true; thread { val s=try{probe(d)}catch(e:Exception){"RAW READ FAILED\n${e.message}"}; runOnUiThread { progress.visibility=ProgressBar.GONE; progress.isIndeterminate=false; status.text=if(s.startsWith("RAW READ SUCCESS")) "Raw sector access confirmed" else "Raw read failed"; resultText.text=s } } }
    private fun probe(d:UsbDevice):String { val intf=mass(d)?:error("No interface"); val e=eps(intf)?:error("No endpoints"); val c=usbManager.openDevice(d)?:error("Open failed"); try { if(!c.claimInterface(intf,true))error("Claim failed"); val cap=capacity(c,intf,e.first,e.second,1); val s=read(c,intf,e.first,e.second,0,1,cap.size,2); return "RAW READ SUCCESS\nBlocks: ${cap.blocks}\nBlock size: ${cap.size}\nSignature: ${String.format("%02X%02X",s[510].toInt() and 255,s[511].toInt() and 255)}\nNo write command sent." } finally { try{c.releaseInterface(intf)}catch(_:Exception){}; c.close() } }
    ...
    private fun scan(d:UsbDevice):String {
        val intf=mass(d)?:error("No interface"); val e=eps(intf)?:error("No endpoints"); val c=usbManager.openDevice(d)?:error("Open failed")
        var rec=0;var cand=0;var rej=0;var bytes=0L;var tag=100;var jpeg:ByteArrayOutputStream?=null;var prev=-1
        try {
            if(!c.claimInterface(intf,true))error("Claim failed"); val cap=capacity(c,intf,e.first,e.second,tag++); var lba=0L
            while(lba<cap.blocks&&!scanCancelled) {
                val n=minOf(SECTORS_PER_READ.toLong(),cap.blocks-lba).toInt(); val x=read(c,intf,e.first,e.second,lba,n,cap.size,tag++)
                for(k in x.indices) { val v=x[k].toInt() and 255; val cur=jpeg; if(cur==null) { if(prev==0xFF&&v==0xD8&&k+1<x.size&&(x[k+1].toInt() and 255)==0xFF) { jpeg=ByteArrayOutputStream(256*1024); jpeg!!.write(0xFF); jpeg!!.write(0xD8); cand++ } } else { if(!(cur.size()==2&&v==0xD8))cur.write(v); if(cur.size()>MAX_JPEG_BYTES){jpeg=null;rej++} else if(prev==0xFF&&v==0xD9){val a=cur.toByteArray();if(valid(a)&&save(a,rec+1))rec++ else rej++;jpeg=null} }; prev=v }
                lba+=n; bytes+=x.size
                if((lba/SECTORS_PER_READ)%64L==0L||lba>=cap.blocks){val p=((lba*1000)/cap.blocks).toInt();val rr=rec;runOnUiThread{progress.progress=p;status.text="Deep scan ${String.format(Locale.US,"%.1f",p/10.0)}% — $rr JPEGs recovered"}}
            }
            val g=bytes/(1024.0*1024*1024)
            return if(scanCancelled) "DEEP SCAN CANCELLED\nScanned ${String.format(Locale.US,"%.2f",g)} GiB\nRecovered: $rec" else "DEEP SCAN COMPLETE\nScanned ${String.format(Locale.US,"%.2f",g)} GiB\nJPEG candidates: $cand\nRecovered JPEGs: $rec\nRejected/incomplete: $rej\nSaved to Pictures/PhotoRescue."
        } finally { try{c.releaseInterface(intf)}catch(_:Exception){}; c.close() }
    }

    private fun read(c:UsbDeviceConnection,intf:UsbInterface,i:UsbEndpoint,o:UsbEndpoint,lba:Long,n:Int,bs:Int,tag:Int):ByteArray { val q=ByteArray(10);q[0]=0x28;q[2]=((lba ushr 24) and 255).toByte();q[3]=((lba ushr 16) and 255).toByte();q[4]=((lba ushr 8) and 255).toByte();q[5]=(lba and 255).toByte();q[7]=((n ushr 8) and 255).toByte();q[8]=(n and 255).toByte();return scsi(c,intf,i,o,q,n*bs,tag) }
    private fun scsi(c:UsbDeviceConnection,intf:UsbInterface,i:UsbEndpoint,o:UsbEndpoint,cdb:ByteArray,len:Int,tag:Int):ByteArray { var last:Exception?=null; repeat(2) { try{return scsiOnce(c,i,o,cdb,len,tag)} catch(e:Exception){last=e;botReset(c,intf,i,o);SystemClock.sleep(100)} }; throw last?:IllegalStateException("USB command failed") }
    private fun scsiOnce(c:UsbDeviceConnection,i:UsbEndpoint,o:UsbEndpoint,cdb:ByteArray,len:Int,tag:Int):ByteArray { val w=ByteBuffer.allocate(31).order(ByteOrder.LITTLE_ENDIAN);w.putInt(0x43425355);w.putInt(tag);w.putInt(len);w.put(0x80.toByte());w.put(0);w.put(cdb.size.toByte());w.put(cdb);while(w.position()<31)w.put(0);if(c.bulkTransfer(o,w.array(),31,5000)!=31)error("CBW transfer failed");val d=ByteArray(len);var got=0;while(got<len){val n=c.bulkTransfer(i,d,got,minOf(16384,len-got),10000);if(n<=0)error("Data transfer failed at $got/$len");got+=n};val s=ByteArray(13);if(c.bulkTransfer(i,s,13,5000)!=13)error("CSW transfer failed");val b=ByteBuffer.wrap(s).order(ByteOrder.LITTLE_ENDIAN);if(b.int!=0x53425355)error("Bad CSW signature");if(b.int!=tag)error("CSW tag mismatch");val residue=b.int;val st=b.get().toInt() and 255;if(st!=0||residue!=0)error("SCSI status=$st residue=$residue");return d }
    ...
    private fun save(a:ByteArray,n:Int):Boolean {
        return try {
            val v=ContentValues().apply{put(MediaStore.Images.Media.DISPLAY_NAME,"Recovered_${String.format(Locale.US,"%05d",n)}.jpg");put(MediaStore.Images.Media.MIME_TYPE,"image/jpeg");put(MediaStore.Images.Media.RELATIVE_PATH,Environment.DIRECTORY_PICTURES+"/PhotoRescue");put(MediaStore.Images.Media.IS_PENDING,1)}
            val u=contentResolver.insert(MediaStore.Images.Media.EXTERNAL_CONTENT_URI,v) ?: return false
            contentResolver.openOutputStream(u)?.use{it.write(a)} ?: return false
            v.clear();v.put(MediaStore.Images.Media.IS_PENDING,0);contentResolver.update(u,v,null,null);true
